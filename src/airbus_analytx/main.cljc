(ns airbus-analytx.main
  "Kotodama WASM entrypoint for the airbus_analytx clean-room actor (L4) — Clojure port.
  ...
  facts (`airbus_analytx.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "airbus_analytx")
(def tier "L4")
...
(defn page-limit
  ...
  src/airbus_analytx/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(defn emit-facts
  "EAVT facts for one record: {\"airbus_analytx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "airbus_analytx-compat" :tier tier :entities entities} 200])
...
(defn start! [] :airbus_analytx_compat/ready)
