(ns airbus-analytx.main-test
  "Contract + behavioral test for the airbus_analytx-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "airbus_analytx-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'airbus-analytx.main-test)]
